...
def send_single_email(to_email, subject, message):
    ...
    try:
        ...
        logging.error(f"⚠️ Failed to send email to {to_email}: {str(e)}")
        return False
...
def self_ping():
    ...
    try:
        ...
        if response.status_code == 200:
            logging.info(f"🔄 Self-ping successful to {base_url}")
        else:
            logging.warning(f"🔄 Self-ping returned {response.status_code}")
            
    except Exception as e:
        logging.warning(f"🔄 Self-ping failed: {str(e)}")
        # If self-ping fails, try localhost (for local testing)
        try:
            requests.get("http://localhost:5000/health", timeout=5)
            logging.info("🔄 Local ping successful")
        except:
            pass
...
Automated email from background scheduler"""
        ...
        logging.info(f"📧 Auto-sending test emails to {total_emails} recipients...")
        ...
        logging.info(f"✅ Auto-emails completed: {success_count}/{total_emails} sent")
        return success_count
        
    except Exception as e:
        logging.error(f"⚠️ Error in send_test_emails: {str(e)}")
        return 0
...
Deployed on Render Cloud"""
        ...
        logging.error(f"⚠️ Error sending status report: {str(e)}")
        return 0
...
def run_daily_smtp_test(app):
    ...
    while True:
        try:
            start_time = get_current_time()
            logging.info(f"⏰ Starting auto SMTP test: {start_time.strftime('%Y-%m-%d %H:%M:%S')} IST")
            ...
            logging.info(f"✅ Auto SMTP test completed in {duration:.1f}s")
            logging.info(f"📧 Emails sent to {test_count} recipients")
            
        except Exception as e:
            logging.error(f"⚠️ Auto SMTP test error: {str(e)}")
...
def run_keep_alive_ping():
    ...
    logging.info("🔄 Keep-alive pinger started")
...
def start_smtp_scheduler(app):
    ...
    try:
        ...
        logging.error(f"⚠️ Failed to start SMTP scheduler: {str(e)}")
        return False
